...
int main(void) {
  int qtdCidades = 5, cod[qtdCidades], veiculos[qtdCidades], acidentes[qtdCidades], soma_veiculos = 0, soma_acidentes = 0, soma_acidentes_cidades_menos_2000 = 0, cidades_menos_2000 = 0, i, j, cod_aux_maior, cod_aux_menor, loop;
  float media_veiculos = 0, media_acidentes = 0, menor = 0, maior = 0, media_acidentes_menos_2000 = 0, indice_acidentes[qtdCidades];
  for (i = 0; i < qtdCidades; i++) { // Loop para receber os dados das cidades
    do { // Loop para receber o código da cidade
      loop = 0;
      printf("\nDigite o código da cidade: ");
      scanf("%d", &cod[i]);
      for (j = 0; j < qtdCidades; j++) { // Loop para verificar se o código já foi cadastrado
        if (i != j && cod[i] == cod[j]) {
          loop = 1;
  ...
  media_veiculos = (float) soma_veiculos / qtdCidades;
  media_acidentes = (float) soma_acidentes / qtdCidades;
